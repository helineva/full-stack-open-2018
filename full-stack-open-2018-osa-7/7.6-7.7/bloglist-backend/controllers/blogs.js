...
blogsRouter.post('/', async (request, response) => {
  ...
  try {
    ...
    const opts = [{ path: 'user', select: ['_id', 'username', 'name'] }];
    const populatedBlog = await Blog.populate(savedBlog, opts);
...
blogsRouter.delete('/:id', async (request, response) => {
  ...
  try {
    ...
    if (exception.name === 'JsonWebTokenError') {
      ...
      console.log(exception);
      return response.status(500).json({ error: 'there is a problem...' });
    }
  }
  try {
    ...
    await user.update({ blogs: user.blogs.filter(blogObj => blogObj.toString() !== blogId) });
